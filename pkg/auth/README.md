...
From `ValidateIDToken` the field is only as trustworthy as the token's source. Google's and Apple's `ValidateIDToken` check the issuer, audience, expiry and nonce but do not verify the token's signature, so a token that did not come straight from the provider can carry any claim. On `Exchange` the value comes from the provider over TLS: Google's userinfo response, or the ID token in Apple's token response.
...
That list is keyed on the name a provider reports in `UserInfo.Provider`, and it fails closed. **Any provider that reports itself as `"google"` or `"apple"`, including a consumer's stub, test fake or custom provider, must set `EmailVerified`**, or every sign-in through it that carries an email is refused.

The reference callback (`authhttp.AuthHandlers.Callback`) **refuses** such a sign-in with `403 {"error": "email address not verified by the identity provider", "code": "email_not_verified"}`. The check runs before `FindOrCreateAgent` and before `InviteAcceptor.AcceptInvite`, so nothing is written: no agent, account, credential or session, and an invite stays pending. `InviteService.AcceptInvite` also refuses such a profile itself, with `application.ErrEmailNotVerified`, and the callback answers that error from either credential writer with the same 403. The callback is a browser redirect target, so the browser shows this body as JSON, like the handler's other errors. A profile with no email is not refused, since there is nothing to bind.

**Returning agents are refused too**, even when a credential for the same provider subject already exists. That refusal is the only thing that stops re-entry through a credential written before this check existed: someone signs in with another person's unverified address, the address's real owner later signs in through a trusted issuer and is joined to that same account, and the first person returns through the subject match. The accepted cost is that a genuine account whose provider email is unverified is refused.
...
- A refusal at least says why sign-in failed, with a stable `code` for any client that calls the callback directly. A person who arrives by browser redirect sees the JSON body, since no page reads the code today. A callback that redirects as though it succeeded while withholding sign-in says nothing at all.

**Writing a credential outside the reference callback.** `InviteService.AcceptInvite` guards itself. `FindOrCreateAgent` does not: seeding and asserted sign-in call it with profiles that carry no provider claim, so a refusal inside it would break them. A caller that writes a google or apple credential through `FindOrCreateAgent` must check `UserInfo.HasUnverifiedEmail()` first and refuse, as the reference callback does.
