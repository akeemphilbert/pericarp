...
### 2026-07-24: Event-store migration tool — export/import CLI + HTTP (epic #60)

- New driver-free `pkg/eventsourcing/migration` package: `Export(ctx, src, w, opts)` streams a store's global feed (`ReadAfter` in `Position` order) to newline-delimited JSON (one `EventEnvelope` per line, preceded by a version header); `Import(ctx, dst, r, opts)` appends in file order with `expectedVersion -1`. Modelled as **export → portable file → import** so source and destination need not be reachable at once and may use different backends. Global order is preserved by *append order* (the destination reassigns `Position`), not by copying positions; payloads are copied as opaque JSON (no schema upcasting). `ExportOptions.FromPosition` resumes an interrupted export; `ImportOptions.SkipExisting` (via `GetEventByID`) makes re-runs idempotent; both carry optional `Progress` callbacks. Depends only on the `domain.EventStore` interface — unit-tested with two `MemoryStore`s.
- New `cmd/pericarp` binary (the previously-empty placeholder) confines all DB-driver deps so the core library stays lean: stdlib-`flag` subcommands `export`/`import`, a backend selector `openStore(spec)` for sqlite (glebarez), postgres (gorm), and dynamo (aws sdk), and `serve` — a stdlib `http.ServeMux` exposing the same operations as **async jobs** (`POST /export|/import` → `202 {id}`, `GET /jobs/{id}` to poll, `GET /export/{id}/download`). Request bodies carry DB credentials, so `serve` binds loopback by default and gates every route except `/healthz` on `PERICARP_MIGRATE_TOKEN` (constant-time bearer check). `make build-cli` → `bin/pericarp`.
- Scope: copies **events only** — destination projections are rebuilt by the destination app's own subscribers (projection handlers are app-specific, not in core); event schemas are not upcast; non-event-sourced tables are not carried. Documented in `cmd/pericarp/README.md`.
- **Why:** operators need to migrate a pericarp app's data to another instance — possibly on a different backend — without a DB-level dump/restore (which cannot cross backends and reassigns global positions). The event store is the source of truth, so moving the feed and letting the destination rebuild its projections is the faithful, backend-agnostic path. No new module dependencies (reuses the existing gorm/sqlite/postgres/aws + ksuid deps).

---

### 2026-08-09: Auth sessions are scoped to an account at sign-in (#68)

- **The defect:** `AuthSession` rows were never scoped to an account, so `auth.Identity.ActiveAccountID` was empty on every request served through `RequireAuth`. `AuthSession.With` took no `accountID`, and `ScopeToAccount` — the only writer of the field — had zero production callers. `ResourceOwnershipFromCtx` rejects an identity with no active account, so no consumer could tag a resource with the caller's tenant on session auth at all. `SoftAuth` (in weos, not here) populated the field correctly from `FindByMember`, which is why every local run looked healthy and only real deployments hit it.
- **The account flows from sign-in; it is not resolved inside `CreateSession`.** This is the load-bearing design decision. `AcceptInvite` adds an invited agent to the *inviter's* account and never creates a personal one, so an internal `FindPersonalByMember` would fix everyone who signed up normally and leave every invited agent broken — a partial fix that looks complete in any testing done with a normal signup account. The account already existed at the call site (`handlers.go` resolves it, then used it two statements later) and was simply dropped.
- **Breaking by choice, not by accident.** `CreateSession` and `AuthSession.With` gained a positional `accountID`; `AuthenticationService` gained `ScopeSessionToAccount`; `SwitchActiveAccountHandler` gained `sessions`/`authService`. A variadic option was considered and rejected: it would let every existing call site keep compiling *and keep silently dropping the account*, which is the defect itself. The compiler is the forcing function. Repo was at `v1.0.0-beta.3`.
- **Sign-in is permissive, requests are strict.** An unresolvable account yields an unscoped session rather than a failed login callback; `RequireAuth` then refuses that session with 401 and a machine-readable `code: "unscoped_session"` (an expired session is fixed by signing in again, an unscoped one is not — without the code, clients loop). No backfill migration: every pre-existing `auth_sessions` row is unscoped, so deploying logs everyone out, and a mixed rolling deploy produces a login loop until the fleet converges.
- **`SessionCreated` carries the account and `ApplyEvent` restores it.** A field-only assignment would have produced a correct database row and a wrong event stream. Note this is verified at the aggregate level only: `AuthSession`'s events are never appended to an event store — unlike `Agent`/`Account`/`Credential`/`Invite`, nothing commits them through a unit of work, nothing replays them, no subscriber consumes them. Pre-existing and out of scope here, but the account now depends on `ApplyEvent` correctness that production never exercises. Worth settling deliberately.
- **First Gherkin in the repo.** `godog` added as a direct test-only dependency with one suite at `pkg/auth/acceptance_test.go` binding `pkg/auth/features/*.feature` (23 scenarios, 154 steps) against the *real* `DefaultAuthenticationService` over sqlite-backed gorm repositories and real middleware — never a stubbed service. The existing `TestRequireAuth_ValidSession_InjectsIdentity` stubbed the service and fed it an account the real service was structurally incapable of producing, which is why this defect passed CI; an acceptance suite built the same way would have reproduced the blind spot faithfully.
- **Also fixed, from a premortem pass:** membership-lookup failures no longer fail session validation (callers map that to 401, so a database blip would have logged everyone out); `FindByMember` gained a deterministic `ORDER BY` (the fallback account varied between logins); the switch handler mints the token before moving the session, answers 403 rather than 500 on a refused membership, and refreshes the cookie's account; token issuance is skipped when unscoped (a token with no active account reproduced this defect on the JWT routes, which admit it).

---

### 2026-09-01: Event-store compaction — one full-state event per aggregate + JSONL archive (#81)

- **New `pkg/eventsourcing/compaction` package.** `Compact(ctx, store, opts)` takes a watermark (global `position`), a `StateProvider`, and an archive; for every aggregate with events at or below the watermark it appends one compaction event of the caller's type carrying the full state, with metadata `compacted_from` (that aggregate's lowest retired position) and `compacted_to` (the watermark), at `sequence max + 1`. Retired events go to an archive in the same newline-delimited JSON shape `migration.Export` writes (it reuses `migration.FormatVersion`), resumable via `opts.FromPosition`. `Retain` (event types, a type predicate, a not-before time) holds events back from both compaction and the archive — this story provides the hook only; the policy belongs to the retention story.
- **The ordering is the whole safety argument.** Per batch: write the archive → fsync → append the compaction events → delete-and-record-the-manifest in one transaction. Every failure before the delete leaves the store exactly as it was; every delete happens with the archive already durable. Because deletes come last, positions are never reused — a retired position is a permanent gap in the global feed. The whole plan, including every `StateProvider` call, is built before the first write, so a provider failure aborts with nothing archived and nothing appended.
- **New optional store capability `domain.CompactableEventStore`** (`RetireEvents` + `Compactions`, with `domain.CompactionManifest` and `ErrCompactionNotSupported`), implemented by `MemoryStore` and `GormEventStore`; `FileStore` and `DynamoEventStore` deliberately do not, so compaction refuses them before reading a single event. Manifests live in a new `compactions` table (GORM automigrated) / an in-memory slice, written in the delete's transaction, so a second run resumes from the highest recorded `to_position` rather than repeating.
- **Two bugs the deletes exposed.** (1) Position reuse: the single-writer path assigned `MAX(position)+1` over surviving rows, which falls back down once rows are deleted and would re-issue a position a feed reader had already consumed — both stores now keep a high-water mark that only rises (the GORM store takes the max of the events table *and* the recorded manifests). (2) `BaseEntity.applyEventInternal` refused any first applied event whose sequence number was not 1, which made a compacted aggregate unrehydratable; an entity with no state now accepts any first sequence `>= 1`, and nothing else was loosened — a gap after state exists is still refused, and `RestoreBaseEntity` keeps strict ordering.
- **Two re-run rules keep snapshots from stacking.** An aggregate whose only candidate events are a previous run's compaction events is skipped entirely (otherwise a later run archives a snapshot and writes an equivalent one straight back). An aggregate whose last event already compacts to this watermark gets no second snapshot, which is what makes the retry after a failed delete converge.
- **Contract-first.** The 36-pickle godog suite (`pkg/eventsourcing/compaction/features/event-store-compaction.feature`, tag `@compaction`) was committed before any implementation and runs once per store kind — memory and SQLite — against real stores, with only the failures the contract names injected. An observing store/archive wrapper records the write/sync/append/delete order, so the durability ordering is proved rather than asserted. The unsupported-store outline uses the real `FileStore` and a real DynamoDB Local container (skipped without Docker, as the DynamoDB store's own tests already do). Two decisions the contract fixed and the code follows: a straddling aggregate is snapshotted as it stands *now* (including events above the watermark), because the compaction event is applied last on replay; and "the last event is a delete" defaults to types ending in `Deleted`, overridable via `opts.IsDelete`.
