...
### 2026-09-15: Google and Apple carry email_verified; the callback refuses an unverified email

- `application.UserInfo` gained `EmailVerified bool`. Google reads the `email_verified` claim from its userinfo response (`Exchange`, `RefreshToken`) and its ID token (`ValidateIDToken`); Apple reads it from the ID token, where it arrives as a boolean or as the string `"true"` / `"false"`. One `emailVerifiedClaim` decoder maps anything else — absent, null, a number, an unrecognised string — to false, never true, and never fails the token.
- For every other provider the field is always false and means "not reported", so the field alone cannot drive a refusal. `UserInfo.HasUnverifiedEmail()` holds the list of providers that report verification (google, apple) and is the one predicate to act on, so a consumer running its own callback and the reference callback cannot disagree about which sign-ins it covers.
- The reference callback refuses with `403` and `code: "email_not_verified"` before `FindOrCreateAgent` or `AcceptInvite`, so nothing is written. **Refuse, not skip:** `FindOrCreateAgent` writes the agent, the account and the credential together, and `CreateSession` needs the credential ID, so a sign-in without a credential would strand an agent and an account with no way back in. A profile with no email is not refused: there is nothing to bind.
- **Returning agents are refused too**, even when a credential for the same provider subject exists. A returning sign-in stores no new email, so that is not the reason. The refusal is the only thing that stops re-entry through a credential written before this check existed: someone signs in with another person's unverified address, the real owner later signs in through a trusted issuer and is joined to that same account, and the first person returns through the subject match. Accepted cost: a genuine account whose provider email is unverified is refused.
- **Where the guard lives:** `InviteService.AcceptInvite` refuses with the exported `ErrEmailNotVerified` itself, so every caller is covered, and the callback maps that error to the same 403. `FindOrCreateAgent` does not guard, because seeding and asserted sign-in call it with profiles that carry no provider claim; a caller that writes a google or apple credential through it must check `HasUnverifiedEmail()` first.
- **Trust limit:** Google's and Apple's `ValidateIDToken` do not verify the token signature, so `EmailVerified` from that path is only as trustworthy as the token's source. The refusal body is JSON shown in the browser, since the callback is a redirect target. The refusal log names the provider and its subject id, never the email; Google logs one Debug line when its userinfo response omits the claim.
- **Behaviour change for consumers:** a stub or custom `OAuthProvider` whose `UserInfo.Provider` is `"google"` or `"apple"` must now set `EmailVerified: true`, or the reference callback refuses it. Google and Apple now hold their `Exchange` endpoints in fields, as Facebook does, so their tests reroute to httptest. Repo was at `v1.0.0-beta.5`.
- **Why:** invites and email-based account binding trust a credential's email. A credential written for an address the provider never verified lends that trust to whoever typed the address.
